...
#include "LTDeletePlaneInfoNotProduct.h"
...
void TModuleMngr::WhatCanYouCreate(std::list<s_wcyc_container_class> &list,
   ...
   if (manager)                                                               {
      ...
      if ((stricmp(name, typeid(RaydenBoard).name()) == 0 || firstLevel) )    {
         s_wcyc_container_class s1(typeid(RaydenBoard).name());
         s1.AddToList(list);
      }
      ...
      if (stricmp(typeid(TMoveCtrl).name() , name) == 0  || firstLevel)       {
         s_wcyc_container_class s1(typeid(TMoveCtrl).name() );
         s1.AddToList(list);
         s_wcyc_container_class s2(typeid(TMoveCtrlView).name() );
         s2.AddToList(list);
      }
      ...
      if ( stricmp(name, typeid( TIObjectAnalizer).name()) == 0 )             {
         s_wcyc_container_class s1( typeid( TCalcBadPositioned).name() );
         s1.AddToList( list );
      }
...
TDataManager *TModuleMngr::CreateDataMngr(wchar_t *name, char* code,
   ...
   if (stricmp(code, typeid(TAnaDefCommonMethods).name()) == 0)
      if (mnc)
         return new TAnaDefCommonMethods(name, mnc);
   ...
   if ( stricmp( code, typeid( TMoveCtrl).name())==0)                         {
      if ( p)
         return new TMoveCtrl( name, p);
   }
   if ( stricmp( code, typeid( TMoveCtrlView).name())==0)                     {
      if ( p)
         return new TMoveCtrlView( name, p);
   }
   if ( stricmp( code, typeid( TCalcBadPositioned).name())==0)                {
      if ( p)
         return new TCalcBadPositioned( name, p);
   }
