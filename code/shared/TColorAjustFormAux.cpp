...
void __fastcall TColorAjustFormAux::C1Click(TObject *Sender) {
   ...
   // Cambiar el aspecto del boton si se pulsa o despulsa
	...
	if(hAvg->Enabled() && celda->TabOrder >= columns) {
	...
	for(int i = 0; i < m_ltBeltDiscrMngrs.size(); i++) {
		...
		LTBeltDiscr_RGBHSI *ltBeltDiscrMngr = m_ltBeltDiscrMngrs[i];

		if(!ltBeltDiscrMngr || !ltBeltDiscrMngr->Enabled())
			continue;

		const wchar_t* name1 = ltBeltDiscrMngr->GetLangName();
		const wchar_t* name2 = preclassificMngr->GetLangName();
