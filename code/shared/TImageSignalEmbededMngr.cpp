...
/**
    ...
    @return
 */
TImageSignalEmbededMngr::TImageSignalEmbededMngr(const wchar_t *name, TProcess *_parent)
...
/**
 Destructor
 */
TImageSignalEmbededMngr::~TImageSignalEmbededMngr()                                         {
   if (m_daSignal != NULL)                                                    {
	  for(int i = 0 ; i < MAX_SIGNALS; i++)                                   {
...
/**
 DataModifiedPost
 */
void __fastcall TImageSignalEmbededMngr::DataModifiedPost(TData* da)                 {
...
/**
 MyStop
 */
bool TImageSignalEmbededMngr::MyStop() {
...
/**
 MyInit
 */
bool TImageSignalEmbededMngr::MyInit() {
...
/**
 MyGo
 */
bool TImageSignalEmbededMngr::MyGo() {
...
/**
 MyProcess
 */
TProcessData * TImageSignalEmbededMngr::MyProcess(TProcessData *processData) {
   if(m_closed)                                                                {
	  return processData;
   }
   ...
   __int64 t = m_img->GetTime();
   __int64 inc = (t - m_lastTime);
   __int64 threshold = m_daThresholdMs->AsInt();

   if (/*inc > 0 && */(inc / 1000) < threshold)				   {
	   return processData;
   }
...
void TImageSignalEmbededMngr::SetClosed(bool aux){
	m_closed = aux;
}
...
void TImageSignalEmbededMngr::ShowFormImageSignal()                                  {
...
//      if (imageSignalMngr)                                                    {
		 ...
		 if(m_signalForm == NULL)                                             {
			...
			m_signalForm = new TImageSignalEmbededForm(NULL, this);
		 }
		 // Show form
		 m_signalForm->Show();
		 SetClosed(false);
...
void TImageSignalEmbededMngr::SetChart(TChart *chart) {
	 m_lastTime = 0;
	 if(m_signalForm == NULL)                                             {
		m_signalForm = new TImageSignalEmbededForm(NULL, this);
	 }
	 m_signalForm->SetChart(chart);
	 m_signalForm->setScaleDepth(m_daScaleDepth->AsInt());
