...
type
  ...
  TAdvSmoothPopup = class(TComponent, ITMSStyle)
  private
...
begin
  ...
  FooterR := MakeRect(rt.X, rt.Y + rt.Height - FooterHeight, rt.Width, FooterHeight);

  HeaderR := MakeRect(rt.X, rt.Y, rt.Width, HeaderHeight);
  ...
  begin
    ...
    FHeaderCaptionRect := MakeRect(HeaderR.X + mgr , HeaderR.Y + (HeaderR.Height / 2) + mgr , HeaderR.Width - mgr * 2, (HeaderR.Height / 2) - mgr * 2);
  end
  else
    FHeaderCaptionRect := MakeRect(ptl.X + mgr, HeaderR.Y + (HeaderR.Height - hr.Height) / 2, totalh - mgr, hr.Height);
  ...
  begin
    ...
    FFooterCaptionRect := MakeRect(FooterR.X + mgr , FooterR.Y + mgr , FooterR.Width - mgr * 2, (FooterR.Height / 2) - mgr * 2);
  end
  else
    FFooterCaptionRect := MakeRect(pbl.X + mgr, FooterR.Y + (FooterR.Height - fr.Height) / 2, totalf - mgr, fr.Height);
...
begin
  ...
  p := TGPPen.Create(MakeColor(Opacity - 50, clBlack), 2);
  g.DrawPath(p, pth);
  p.Free;
  pth.Free;
