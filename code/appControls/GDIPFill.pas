...
uses
  Windows, Graphics, Classes, SysUtils, Math, Clipbrd, IniFiles, AdvGDIP
  {$IFDEF DIRECT2D}
...
begin
  ...
  begin
    path := CreateRoundRectangle(MakeRect(hs + x,vs + y, Width - (hs * 2), Height - (vs * 2)), 8, RoundingType, false);
  ...
  begin
    ...
    case PictureAlignment of
      ...
      taRightJustify: l := (Width - 4 - hs - Picture.Width);
      taCenter: l := (Width - Picture.Width) div 2;
    end;
    ...
    begin
      case ly of
        ...
        begin
          case PictureAlignment of
            taLeftJustify: l := l + x;
            taRightJustify: l := Round(pt.X - Picture.Width - Spacing);
            taCenter: l := Round(pt.X - (pt.X + Picture.Width - x) / 2);
          end;
        ...
        begin
          case PictureAlignment of
            ...
            taCenter: l := Round(pt.X + tw + (Width - (pt.X + tw) - Picture.Width) / 2);
          end;
          Picture.GDIPDraw(g, Bounds(l, t + y, Picture.Width, Picture.Height));
        end;
        blPictureBottom:
    ...
    case Layout of
      blPictureLeft: textr := Makerect(l + x + Picture.Width + Spacing, textr.Y, Width - 8 - (hs * 2) - Picture.Width - l - x - Spacing, textr.Height);
      blPictureRight: textr := Makerect((hs * 2) + 8, textr.Y, l - Spacing - (hs * 2) - 8 - x, textr.Height);
    end;
  end;
