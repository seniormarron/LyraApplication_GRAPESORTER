...
procedure GetObjectLocation(var x, y: Single; rectangle: TGPRectF; objectwidth, objectheight: integer; location: TAdvSmoothImageListBoxLocation);
var
  w, h, tw, th: integer;
begin
  tw := objectwidth;
...
procedure TAdvSmoothImageListBox.AddImageLocationsFromFolder(AFolder:
  String; SetImageCaption: Boolean = false);
var
  SR: TSearchRec;
    ...
    begin
      ...
      begin
        ...
        if Navigator.Visible then
          Draw(Navigator.GetNavigatorRect(fillr, CaptionLocation, ftext), FHovF, FSelF);
      end;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        //NavigatorButtons
        if Navigator.Visible then
          Draw(Navigator.GetNavigatorRect(fillr, CaptionLocation, htext), FHovH, FSelH);
      end;      
    end;
  end;
end;
...
begin
  ...
  //OutputDebugString(PChar('--Vis Item Count: ' + InttoStr(FVisibleItemCount) + 'VC: '+ Inttostr(FVisibleCol)+ ' VR: ' + Inttostr(FVisibleRow)));
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FSelectedItemIndex <> -1) and (FindItemOnXY(X, Y) <> -1) and (FDisplayList.Count > 0) and (Items.Count > 0) then
      begin
        FMouseUp := false;      
