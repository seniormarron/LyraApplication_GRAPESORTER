...
type
  ...
  private
    FItemRect, FItemRectOrig: TGPRectF;
    FOwner: TAdvSmoothStepControl;
    FItemObject: TObject;
    FTag: Integer;
  ...
  TStepMode = (mActive, mProcessed, mInactive);
  ...
  TAdvSmoothStepControl = class(TCustomControl, ITMSStyle)
  private
...
begin
  ...
  begin
    ...
    if StepActions[idx].GetAnchorAt(X, Y) <> '' then
...
begin
  ...
  if (idx >= 0) and (idx <= StepActions.Count - 1) then
  begin
    str := StepActions[idx].GetAnchorAt(X, Y);
    if str <> '' then
    begin
      if Assigned(OnStepAnchor) then
        OnStepAnchor(Self, idx, str);
    end
    else
    begin
     ActiveStep := idx;
...
begin
  ...
  begin
    ...
    begin
      if PtInGPRect(StepActions[I].FItemRect, Point(X, Y)) then
      begin
        Result := I;
        break;
      end;
...
var
  ...
  g: TGPGraphics;
begin
  r  := Makerect(0, 0, FOwner.Width - 1, FOwner.Height - 1);
  bmp := TBitmap.Create;
