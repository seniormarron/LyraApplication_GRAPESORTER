...
type
  ...
  TAdvSmoothSplashScreen = class(TComponent, ITMSStyle)
  private
    FAutoShow: Boolean;
    ...
    FStayOnTop: Boolean;    // Timer used to force focus to main form
    function GetVersion: string;
    procedure SetVersion(const Value: string);
    function GetVersionNr: Integer;
  ...
  protected
    ...
    procedure RefreshItem(Item: TSplashItem);
    procedure RefreshRect(R: TRect);
    procedure Changed;
...
procedure TSplashItem.Refresh;
begin
  if (TSplashItems(Collection).Owner is TAdvSmoothSplashScreen) then
  begin
    TAdvSmoothSplashScreen(TSplashItems(Collection).Owner).RefreshItem(Self);
  end;
end;
...
procedure TAdvSmoothSplashScreen.RefreshRect(R: TRect);
begin
...
procedure TAdvSmoothSplashScreen.RefreshItem(Item: TSplashItem);
var
  R: TRect;
begin
...
var
  pt, pt2: TPoint;
begin
  if Assigned(AForm) and FShowing then
  begin
    Pt.x := 0;
    Pt.y := 0;
