...
type
  ...
  published
    ...
    property AnnotationAutoPosition
      : TAnnotationAutoPosition read FAnnotationAutoPosition write
      SetAnnotationAutoPosition default apNone;
    property SectionCaptionFont: TFont read FSectionCaptionFont write
      SetSectionCaptionFont;
  ...
  protected
    ...
    function IndicatorAtXY(X, Y: Integer; var OnAnnotation: Boolean;
    ...
    function GetAnimatedMargin(DivisionType: TAdvsmoothTimelineDivisionType)
      : Double;
    function GetDivisionValue(DivisionType: TAdvsmoothTimelineDivisionType)
      : Double;
    ...
    procedure DoSectionDown(Sender: TObject;
      section: TAdvSmoothTimeLineBarSection); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  ...
  published
    ...
    property OnIndicatorHint
      : TAdvSmoothTimeLineIndicatorHint read FOnIndicatorHint write
      FOnIndicatorHint;
    property OnIndicatorPositionChanged
      : TAdvSmoothTimeLineIndicatorPosition read FOnIndicatorPositionChanged
      write FOnIndicatorPositionChanged;
    property OnSectionPositionChanged: TAdvSmoothTimeLineSectionPosition read
      FOnSectionPositionChanged write FOnSectionPositionChanged;
    property OnIndicatorPositionChanging
      : TAdvSmoothTimeLineIndicatorPosition read FOnIndicatorPositionChanging
      write FOnIndicatorPositionChanging;
    property OnSectionPositionChanging: TAdvSmoothTimeLineSectionPosition read
      FOnSectionPositionChanging write FOnSectionPositionChanging;
    property OnIndicatorClick
      : TAdvSmoothTimeLineIndicatorClick read FOnIndicatorClick write
      FOnIndicatorClick;
    property OnSectionClick
      : TAdvSmoothTimeLineSectionClick read FOnSectionClick write
      FOnSectionClick;
    property OnDivisionText
      : TAdvSmoothTimeLineDivisionText read FOnDivisionText write
      FOnDivisionText;
    property OnSubDivisionText
      : TAdvSmoothTimeLineDivisionText read FOnSubDivisionText write
      FOnSubDivisionText;
    property OnDrawTimeLine
      : TAdvSmoothTimeLineDrawEvent read FOnDrawTimeLine write FOnDrawTimeLine;
    property OnScrollTimeLine
      : TAdvSmoothTimeLineScrollEvent read FOnScrollTimeLine write
      FOnScrollTimeLine;
...
procedure TAdvSmoothTimeLine.BuildSections;
var
  I: Integer;
  r: TGPRectF;
  xs, xe: Double;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FMouseDownOnStartSection and (Index = FSelectedSection) then
      begin
        xs := min(max(FSecStartX, r.X), r.X + r.Width);
        xe := min(max(DateTimeToX(EndTime), r.X), r.X + r.Width);
      end
      else if FOwner.FMouseDownOnEndSection and (Index = FOwner.FSelectedSection)
        then
      begin
        xe := min(max(FSecEndX, r.X), r.X + r.Width);
        xs := min(max(DateTimeToX(StartTime), r.X), r.X + r.Width);
      end
      else if FOwner.FMouseMovedOnSection and FOwner.FMouseMovedOnSection and
        (Index = FOwner.FSelectedSection) then
      begin
        xs := min(max(FSecStartX, r.X), r.X + r.Width);
        xe := min(max(FSecEndX, r.X), r.X + r.Width);
      end
          ...
          begin
            ...
            begin
              if (Range.SubDivisions * Range.Divisions) = 0 then
                ...
                v := Range.RangeFrom +
                  (GetRange / (Range.SubDivisions * Range.Divisions)) * K;
...
begin
  timeliner := GetTimeLineRect;
  w := timeliner.Width;
  if (Range.Divisions = 0) or not(w > 10) or not
...
begin
    ...
    begin
      ...
      if Assigned(FOnSectionPositionChanging) then
        FOnSectionPositionChanging(Self, TimeLineSections[FSelectedSection],
          vstart, TimeLineSections[FSelectedSection].EndTime);
      BuildSections;
      Changed;
    end;
  end
  ...
  begin
    ...
    begin
      ...
      if Assigned(FOnSectionPositionChanging) then
        FOnSectionPositionChanging(Self, TimeLineSections[FSelectedSection],
          TimeLineSections[FSelectedSection].StartTime, vend);
      BuildSections;
      Changed;
    end;
  end
...
begin
  ...
  if FOwner.FMouseMovedOnIndicator and FOwner.FMouseMovedOnIndicator and
