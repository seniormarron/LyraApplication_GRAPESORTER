...
RaydenBoard::RaydenBoard(const wchar_t *name, TDataManagerControl *_parent)
   ...
   m_pNewCfgbyString   = new TData ( this, L"NewCfg_byString", NULL, L" ", 1000, FLAG_NORMAL, L"New config to send. Sin 'S' de inicio ni 'X' de cierre");
   m_pNewCfgbyDatas    = new TData ( this, L"NewCfg_byDatos",  NULL, L" ", 1000, FLAG_NORMAL, L"New config to send. Sin 'S' de inicio ni 'X' de cierre");
   ...
   m_numIlus           = new TData ( this, L"NumIlus", 	     NULL, L"08", 50, FLAG_NORMAL, L"Formato 'XX' (ej: 04). Número de iluminaciones a utilizar");
      ...
      case RaydenBoardNextCmd::SETCFG :
		   ...
		   //	   if( prevStatus !=  RaydenBoardStatus::UNKOWN){
				  ...
				  if ( retCfg)   											  {
					 ...
					 if (m_typeNewCfg->AsInt()== 0 ) {          				// Cogemos la config de m_pNewCfgbyString
						if ( m_pNewCfgbyString->AsText( cfg, size)) 		  {
					 ...
					 else 							 						  { // Cogemos la config de m_pNewCfgbyDatas
						FillCfgStringbyDatas();            				   		// Montamos el string segun configuracion por Tdatas
						if (m_pNewCfgbyDatas->AsText( cfg, size))
							ret = SendCmdToRaydenBoard( cfg, true);
							LOG_INFO2(L"Se ha enviado una configuracion nueva a la RaydenBoard by 'Datas'", GetLangName());
					 }
				  }
				  else              										  {
...
//
//   if ( m_pNewCfgbyDatas->SetAsText( cfg)) {
//
...
//---------------------------------------------------------
/*
FillCfgStringbyDatas
*/
void RaydenBoard::FillCfgStringbyDatas()						  		  	{
   ...
   // Escribimos la cadena en el TData
   m_pNewCfgbyDatas->SetAsText(characters.c_str());
